...
def generate_counterfactual(
    ...
    def finish(result: dict) -> dict:
        ...
        if scorer is not None:
            result["disclaimer"] = "Simulation only, not a hiring guarantee. Semantic relevance and other candidates stay fixed; explicit coverage, evidence, graph coverage and the missing-required penalty are recomputed."
    ...
    if scorer is not None:
        # The capped required-skill penalty makes gains non-additive. Enumerate
        # small sets using the actual FINAL score, including penalty relief.
        changes = _enumerate_changes(jd, candidate)
        def projected(selected):
            working = candidate
            for change in selected:
                working = _apply_change(working, change)
            rescored = scorer(jd, copy.deepcopy(working)) or {}
            values = rescored.get("scores", rescored)
            return float(values.get("final_score", float(values.get("base_score", current_base)) - penalty))

        best_changes, best_score = (), current_final
        for size in range(1, min(MAX_IMPROVEMENTS, len(changes)) + 1):
            for selected in combinations(changes, size):
                value = projected(selected)
                if value > best_score:
                    best_changes, best_score = selected, value
            if best_score >= needed:
                break
        improvements = []
        previous = current_final
        for i, change in enumerate(best_changes):
            value = projected(best_changes[:i + 1])
            improvements.append({"requirement_id": change["requirement_id"], "requirement": change["requirement_name"], "change": change["change"], "kind": change["kind"], "importance": change["importance"], "estimated_gain": round(value - previous, 4)})
            previous = value
        result = finish({
            "target_rank": target_rank, "target_score": target_score,
            "suggested_improvements": improvements, "projected_score": best_score,
            "current_score": current_final, "score_gap_to_target": max(0.0, needed - current_final),
            "reached_target": best_score >= needed, "already_in_top_3": already_top3,
            "message": (f"{len(improvements)} simulated improvement(s) would exceed the current rank #{target_rank} score." if best_score >= needed else f"No set of up to {MAX_IMPROVEMENTS} eligible improvements reaches the current rank #{target_rank} score."),
        })
        return result
